...
class Game(object):
    ...
    def get_state(self)->GameState:
        ...
        if len(self.cards_out)>=1:
            state.last_move_ = Card.vectorized_card_list(self.cards_out[-1][-1])
        else:
            state.last_move_ = []
        if len(self.cards_out)>=2:
            state.last_last_move_ = Card.vectorized_card_list(self.cards_out[-2][-1])
        else:
            state.last_last_move_ = []
        return state
    ...
    def step(self):
        ...
        state, cur_moves, cur_move, cur_desc, self.end, information = player.step(self.get_state()) #返回：在状态state下，当前玩家的出牌、出牌描述、游戏是否结束
        if len(cur_move)==0:
        ...
        return player.player_id, state, cur_moves, cur_move, cur_desc, winner, information
...
class Agent(object):
    ...
    def step(self, state):
        ...
        self.state = state
        self.move,information = self.choose(state)
        self.desc = get_move_desc(self.move)
        end = self.__common_step(self.move, self.desc)
        return state, self.move_list, self.move, self.desc, end, information
